        ...
        <div className="flex flex-col gap-4">
          ...
          >
            <div>
              <div className="font-medium">
                {COPY.adventure.packDrawer.title[lang]}
              </div>
              <div className="text-sm text-clay/70">
                {COPY.adventure.packDrawer.hint[lang]}
              </div>
            </div>
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-sm text-clay/70">
                {COPY.adventure.sponsor.label[lang]}
              </div>
            </div>
...
function PackToggles() {
  ...
  const [local, setLocal] = useState<Record<string, boolean>>(() => {
    ...
    for (const p of EXPANSION_PACKS) base[p.id] = initial[p.id] ?? true;
    return base;
  });

  const set = (id: string, val: boolean) => {
    const next = { ...local, [id]: val };
    setLocal(next);
    save(key, next);
  };
